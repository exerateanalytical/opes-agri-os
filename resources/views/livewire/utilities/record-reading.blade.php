    ...
    @if ($account)
        <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" wire:click.self="close">
            <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-surface p-5 sm:rounded-2xl">
                <h2 class="text-[17px] font-bold text-ink">Record reading</h2>
                <p class="mt-1 text-[13.5px] text-muted">{{ ucfirst($account->utility_type) }}{{ $account->provider_name ? ' · '.$account->provider_name : '' }}</p>

                <form wire:submit="save" class="mt-4 space-y-4">
                    <label class="block">
                        <span class="{{ $labelClass }}">Date</span>
                        <input type="date" wire:model="readOn" class="{{ $inputClass }}">
                        @error('readOn') <p class="mt-1 text-[12.5px] font-medium text-negative">{{ $message }}</p> @enderror
                    </label>
                    <label class="block">
                    ...
                    <label class="block">
                        ...
                        <input type="number" step="0.01" wire:model="consumption" class="{{ $inputClass }}">
                    </label>
                    <label class="block">
