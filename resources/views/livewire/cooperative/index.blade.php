...
<div class="px-5 pb-8 lg:px-6 lg:pt-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
            ...
            <p class="mt-1 text-[14.5px] text-muted">Member registry, contributions, and loans.</p>
        </div>
        @if ($tab === 'members')
        ...
        @else
            @can('cooperative.create')
                <button type="button" wire:click="startAddingLoan"
                        class="tap focusable flex shrink-0 items-center gap-2 rounded-full bg-fill-brand px-5 py-2.5 text-[14.5px] font-semibold text-white transition-opacity hover:opacity-90">
                    <x-icon name="plus" class="size-[18px]" />
                    <span class="sr-only min-[420px]:not-sr-only">Add</span>
                </button>
    ...
    <div class="mt-5 flex flex-wrap gap-2">
        <button type="button" wire:click="$set('tab', 'members')"
                class="tap focusable rounded-full px-4 py-2 text-[13.5px] font-semibold {{ $tab === 'members' ? 'bg-fill-brand text-white' : 'bg-surface-2 text-ink-2 hover:bg-tint-blue hover:text-brand' }}">
            Members
        </button>
        <button type="button" wire:click="$set('tab', 'loans')"
                class="tap focusable rounded-full px-4 py-2 text-[13.5px] font-semibold {{ $tab === 'loans' ? 'bg-fill-brand text-white' : 'bg-surface-2 text-ink-2 hover:bg-tint-blue hover:text-brand' }}">
            Loans
        </button>
    </div>
