    ...
    @if ($animal)
        <div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4" wire:click.self="close">
            <div class="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-surface p-5 sm:rounded-2xl">
                <h2 class="text-[17px] font-bold text-ink">Record health event</h2>
                ...
                <form wire:submit="save" class="mt-4 space-y-4">
                    <label class="block">
                        ...
                        <select wire:model="recordType" class="{{ $inputClass }}">
                    ...
                    <label class="block">
                        ...
                        <input type="text" wire:model="description" autofocus class="{{ $inputClass }}">
                        @error('description') <p class="mt-1 text-[12.5px] font-medium text-negative">{{ $message }}</p> @enderror
                    </label>
                    <div class="grid grid-cols-2 gap-3">
                        <label class="block">
                            <span class="{{ $labelClass }}">Administered on</span>
                            <input type="date" wire:model="administeredOn" class="{{ $inputClass }}">
                        </label>
                        <label class="block">
                            <span class="{{ $labelClass }}">Next due (optional)</span>
                            <input type="date" wire:model="nextDueOn" class="{{ $inputClass }}">
                        </label>
                    </div>
                    <label class="block">
                        <span class="{{ $labelClass }}">Cost (optional)</span>
                        <input type="number" step="0.01" wire:model="cost" class="{{ $inputClass }}">
                    </label>
                    <label class="block">
