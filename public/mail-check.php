...
if ($to !== '' && filter_var($to, FILTER_VALIDATE_EMAIL) && $config !== null) {
    try {
        Mail::raw(
            "This is a test from your OPES AGRI OS install.\n\nIf you are reading it, sending works.",
            fn ($message) => $message->to($to)->subject('OPES AGRI OS mail test'),
        );
...
<title>Mail check · OPES AGRI OS</title><style>
...
if ($working !== [] && $current !== null && ! ($probes[$current]['ok'] ?? false)) {
    ...
    Edit <code>opesagrios/.env</code> to use it:
    <pre>MAIL_HOST=<?= h($pick['host'])."\n" ?>MAIL_PORT=<?= h((string) $pick['port'])."\n" ?>MAIL_SCHEME=<?= $pick['port'] === 465 ? 'smtps' : 'tls' ?></pre>
    then delete <code>opesagrios/bootstrap/cache/config.php</code> if it exists, and reload this page.</div>
<?php } ?>
</div>
