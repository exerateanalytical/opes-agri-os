...
// ---------------------------------------------------------------- locating things
$root = null;
foreach ([__DIR__.'/..', __DIR__.'/../opesagrios'] as $candidate) {
...
if ($root === null) {
    fail('OPES AGRI OS could not find its application files. They belong either in the parent directory of this one, or in a folder named "opesagrios" beside it.');
}
...
<title>{$title} · OPES AGRI OS setup</title><style>
...
if ($action === 'configure') {
    $appName = post('app_name', 'OPES AGRI OS');
    ...
    if ($errors === []) {
        try {
            ...
            $errors[] = 'Could not connect to the database: '.e($e->getMessage())
                .'<br>In cPanel the names are prefixed — asking for <code>opesagrios</code> gives you <code>youruser_opesagrios</code>. Use the full name, and check the user is added to the database in MySQL Databases.';
        }
    }
    ...
    if ($errors === []) {
        try {
            ...
            if ($errors === []) {
                ...
                page('Setup complete', '
                    <div class="done">
                        <p><strong class="ok">OPES AGRI OS is installed.</strong></p>
    ...
    page('Connect your database', $errorHtml.'
        ...
        <form method="post">
          ...
          <label>Business name shown in the app
            <input name="app_name" value="'.e(post('app_name', 'OPES AGRI OS')).'" required></label>
          ...
          <label>Database name
            <input name="db_name" value="'.e(post('db_name')).'" placeholder="youruser_opesagrios" required></label>
          <label>Database user
...
page('Create your administrator', $errorHtml.'
    ...
    <form method="post">
      ...
      <button type="submit">Install OPES AGRI OS</button>
    </form>', 'Step 2 of 2');
