# Deploying OPES AGRI OS to Namecheap shared hosting (cPanel)
...
Either produces `build/opesagrios-YYYYMMDD-HHMM.zip`, about 28 MB. It contains the
...
```
/home/youruser/
├── opesagrios/        the application — never web-served; holds .env
...
2. **Import** tab → **Choose File** → `opesagrios-install.sql` → **Go**.
...
1. **Your database and email** — the database name, user and password you
   ...
   prefixed names**: asking for `opesagrios` gives you `youruser_opesagrios`. The
...
```
cd /home/youruser/opesagrios && /usr/local/bin/php artisan schedule:run >> /dev/null 2>&1
```

**Queue:**

```
cd /home/youruser/opesagrios && /usr/local/bin/php artisan queue:work --stop-when-empty --tries=3 --max-time=50 >> /dev/null 2>&1
```
...
To change the settings later, File Manager → `opesagrios` → right-click `.env` →
...
```
cd /home/youruser/opesagrios && /usr/local/bin/php artisan config:cache >> /dev/null 2>&1
```
...
**"OPES AGRI OS could not find its application files."** The app folder is not named
`opesagrios`, or is not in your home directory beside `public_html`.
...
3. Copy your existing `.env` from `opesagrios` into the new folder.
4. Rename the old folder to `opesagrios-old`, the new one to `opesagrios`.
...
Keep `opesagrios-old` until the new one is proven. Rolling back is renaming two
folders.
