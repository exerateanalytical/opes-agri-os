# Running OPES AGRI OS on Windows at https://opesagrios.test
...
- **Laragon installed** (e.g. the project lives in `C:\laragon\www\opesagrios`) —
...
the domain from the folder name, so `opesagrios` gives `opesagrios.test`.
...
unless you specifically want the `opesagrios.test` hostname.
...
3. Extract the release (or clone the repo) into Herd's parked directory,
   usually `C:\Users\<you>\Herd\`, as a folder named `opesagrios`.
   The folder name becomes the domain, so `opesagrios` gives `opesagrios.test`.
4. In Herd, click **Secure** on the site. That issues a trusted certificate and
   switches it to `https://opesagrios.test`.
...
this is the route for `C:\laragon\www\opesagrios`.

1. Install **[Laragon Full](https://laragon.org/download/)** — it includes PHP,
   nginx/Apache and MySQL.
2. Extract the release into `C:\laragon\www\opesagrios`. The folder name becomes
   the domain: `opesagrios` → `opesagrios.test`. Laragon detects the `artisan` file
...
5. It opens `https://opesagrios.test` once the site answers. Sign in with
   `john@opesware.com` / `password`.

Doing it by hand instead of the installer is the same four things: create the
`opesagrios` database, copy `.env.example` to `.env` with the values in
...
sudo mkdir -p /var/www/opesagrios
...
```sql
CREATE DATABASE opesagrios CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
```
...
**https://opesagrios.test** → `john@opesware.com` / `password`
