...
#
#   opesagrios/      the application — must NOT be web-served, it holds .env
...
mkdir -p "$STAGE/$RELEASE/opesagrios" "$STAGE/$RELEASE/public_html"

APP="$STAGE/$RELEASE/opesagrios"
...
if [ ! -f database/schema/opesagrios-install.sql ]; then
    echo "WARNING: database/schema/opesagrios-install.sql is missing." >&2
...
echo "==> Compressing"
(cd "$STAGE/$RELEASE" && zip -qr "$ARCHIVE" opesagrios public_html)

# Beside the zip, ready to be selected in phpMyAdmin's Import tab.
if [ -f database/schema/opesagrios-install.sql ]; then
    cp database/schema/opesagrios-install.sql "$(dirname "$ARCHIVE")/opesagrios-install.sql"
fi
