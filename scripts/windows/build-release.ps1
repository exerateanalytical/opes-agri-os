...
$app = Join-Path $stageRelease 'opesagrios'
...
$schema = Join-Path $root 'database\schema\opesagrios-install.sql'
if (-not (Test-Path $schema)) {
    Write-Host 'WARNING: database\schema\opesagrios-install.sql is missing. Run: php artisan opes:export-schema' -ForegroundColor Yellow
}
...
if (Test-Path $schema) { Copy-Item $schema (Join-Path $outDir 'opesagrios-install.sql') -Force }
