<#
.SYNOPSIS
    Makes the local OPES AGRI OS install reachable from other devices on the same
...
Write-Host "OPES AGRI OS - LAN / phone access" -ForegroundColor White
...
$ruleName = "OPES AGRI OS LAN Access ($LanPort)"
...
if ($caRoot) {
    $dest = Join-Path ([Environment]::GetFolderPath('Desktop')) 'OPES AGRI OS-local-CA.crt'
    Copy-Item $caRoot $dest -Force
    Ok "Copied to your Desktop as OPES AGRI OS-local-CA.crt"
} else {
...
Write-Host "    1. Send OPES AGRI OS-local-CA.crt from the Desktop to the phone (email, AirDrop, etc.)" -ForegroundColor Gray
...
Write-Host ""
Write-Host "  This runs independently of opesagrios.test - re-run this script any time" -ForegroundColor Gray
